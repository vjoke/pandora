...
fn can_not_sync_from_light_peer() {
	...
	net.peer(1).on_connect(2);
	net.peer(2).on_connect(1);
	net.peer(1).announce_block(light_info.best_hash);
	net.sync_with_disconnected(vec![0].into_iter().collect());
	...
	net.peer(1).receive_message(2, message::generic::Message::BlockRequest(message::generic::BlockRequest {
		id: 0,
