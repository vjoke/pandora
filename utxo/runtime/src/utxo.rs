...
decl_storage! {
    trait Store for Module<T: Trait> as Utxo {
        ...
        UnspentOutputs build(|config: &GenesisConfig<T>| {
            config.initial_utxo
                ...
                .collect::<Vec<_>>()
        }): map H256 => Option<TransactionOutput<T::BlockNumber>>;
    ...
    add_extra_genesis {
        config(initial_utxo): Vec<TransactionOutput<T::BlockNumber>>;
    }
}
...
decl_module! {
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        pub fn execute(origin, transaction: Transaction<T::BlockNumber>) -> Result {
        ...
        fn on_finalize() {
            // TODO: use RuntimeApi to get authorities
...
impl<T: Trait> Module<T> {
    ...
    pub fn check_transaction(transaction: &Transaction<T::BlockNumber>) -> CheckResult<'_> {
        ...
        for input in transaction.inputs.iter() {
            ...
            let temp: Option<TransactionOutput<T::BlockNumber>> = <UnspentOutputs<T>>::get(&input.parent_output);
