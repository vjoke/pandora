...
construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic
	{
		...
		Balances: balances,
		Sudo: sudo,
		Utxo: utxo::{Module, Call, Storage, Event<T>, Config<T>},
	}
);
...
impl_runtime_apis! {
    ...
    impl runtime_api::TaggedTransactionQueue<Block> for Runtime {
        fn validate_transaction(tx: <Block as BlockT>::Extrinsic) -> TransactionValidity {
            ...
            // if let Some(&utxo::Call::execute(ref transaction)) = IsSubType::<utxo::Module<Runtime>>::is_sub_type(&tx.function) {
	...
	impl consensus_primitives::ConsensusApi<Block, AuraId> for Runtime {
		fn authorities() -> Vec<AuraId> {
			Aura::authorities()
		}
	}
