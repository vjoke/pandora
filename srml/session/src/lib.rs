...
use sr_primitives::{BoundToRuntimeAppPublic, KeyTypeId, Perbill, RuntimeAppPublic};
use sr_staking_primitives::SessionIndex;
use support::{decl_event, decl_module, decl_storage, dispatch::Result, ConsensusEngineId};
use support::{
    ensure,
    traits::{FindAuthor, Get, OnFreeBalanceZero},
    Parameter,
};
...
impl<A> OnSessionEnding<A> for () {
    fn on_session_ending(_: SessionIndex, _: SessionIndex) -> Option<Vec<A>> {
        None
    }
}
...
mod tests {
    ...
    use sr_primitives::{testing::UintAuthorityId, traits::OnInitialize};
